...
func (self *Cpu) Get() error {
	load := [C.CPUSTATES]C.long{C.CP_USER, C.CP_NICE, C.CP_SYS, C.CP_SPIN, C.CP_INTR, C.CP_IDLE}
	...
	self.Irq = uint64(load[4])
	self.Idle = uint64(load[5])
...
func fillCpu(cpu *Cpu, load [C.CPUSTATES]C.long) {
	...
	cpu.Irq = uint64(load[4])
	cpu.Idle = uint64(load[5])
}
